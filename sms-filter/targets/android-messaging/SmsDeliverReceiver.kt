...
class SmsDeliverReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Telephony.Sms.Intents.SMS_DELIVER_ACTION) {
            ...
            try {
                ...
                Log.e("SmsDeliver", "Error parsing rules", e)
            }

            if (shouldBlock) classification = "suspicious"
            val eventId = recordSmsEvent(context, sender, classification)
            if (shouldBlock) {
                Log.d("SmsDeliver", "Suspicious SMS delivered on device")
                showSuspiciousSmsNotification(context, eventId)
            } else {
                Log.d("SmsDeliver", "SMS delivered and analyzed on device")
            }
        }
    }
    ...
    private fun recordSmsEvent(
        ...
        try {
            ...
            Log.e("SmsDeliver", "Could not record SMS analysis event", error)
        }
        return eventId
    }
    ...
    private fun showSuspiciousSmsNotification(
        ...
        try {
            ...
            Log.e("SmsDeliver", "Could not show suspicious SMS notification", error)
        }
    }
}
