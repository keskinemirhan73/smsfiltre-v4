...
test('manuel kategori düzeltmeleri SMS analiz sayaçlarını ve haftalık trendi şişirmez', () => {
  const now = new Date(2026, 7, 1, 12, 0, 0, 0);
  const history = [{ status: 'blocked' as const, timestamp: now.getTime(), source: 'manual' as const }];
